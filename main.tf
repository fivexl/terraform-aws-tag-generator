...
locals {
  ...
  git_other_value           = var.git_other != null ? { (local.git_other) = var.git_other } : {}
  terraform_managed_value   = var.terraform_managed != null ? { (local.terraform_managed) = var.terraform_managed } : {}
  terraform_state_value     = var.terraform_state != null ? { (local.terraform_state) = var.terraform_state } : {}
  terraform_other_value     = var.terraform_other != null ? { (local.terraform_other) = var.terraform_other } : {}
  data_pii_value            = var.data_pii != null ? { (local.data_pii) = var.data_pii } : {}
  data_phi_value            = var.data_phi != null ? { (local.data_pii) = var.data_phi } : {}
  data_pci_value            = var.data_pci != null ? { (local.data_pci) = var.data_pci } : {}
  data_owner_value          = var.data_owner != null ? { (local.data_owner) = var.data_owner } : {}
  data_classification_value = var.data_classification != null ? { (local.data_classification) = var.data_classification } : {}
  data_other_value          = var.data_other != null ? { (local.data_other) = var.data_other } : {}
  environment_name_value    = var.environment_name != null ? { (local.environment_name) = var.environment_name } : {}
  environment_type_value    = var.environment_type != null ? { (local.environment_type) = var.environment_type } : {}
  environment_other_value   = var.environment_other != null ? { (local.environment_other) = var.environment_other } : {}
  gc_enable_value           = var.gc_enable != null ? { (local.gc_enable) = var.gc_enable } : {}
  gc_ttl_value              = var.gc_ttl != null ? { (local.gc_ttl) = var.gc_ttl } : {}
  gc_other_value            = var.gc_other != null ? { (local.gc_other) = var.gc_other } : {}
